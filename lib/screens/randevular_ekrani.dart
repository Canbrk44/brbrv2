            ...
            slivers: [
              ...
              SliverAppBar(
                ...
                flexibleSpace: FlexibleSpaceBar(
                  ...
                  background: Stack(
                    ...
                    children: [
                      Image.network(
                        'https://images.pexels.com/photos/3993323/pexels-photo-3993323.jpeg',
                        fit: BoxFit.cover,
                      ),
                      Container(
              ...
              FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (all.isEmpty) {
                    return const SliverFillRemaining(child: Center(child: Text("Henüz randevunuz yok.")));
                  }
                    ...
                    sliver: SliverList(
                      delegate: SliverChildListDelegate([
                        if (aktif.isNotEmpty) ...[
                          ...
                          ...aktif.map((r) => _RandevuKartiPremium(r: r, isAktif: true, db: _dbService, user: _currentUserName, onRefresh: () => setState(() {}))),
                        ],
                        if (gecmis.isNotEmpty) ...[
                          const SizedBox(height: 30),
                          const _BolumBasligi(baslik: "Geçmiş Deneyimler", icon: Icons.history, color: Colors.grey),
                          const SizedBox(height: 15),
                          ...gecmis.map((r) => _RandevuKartiPremium(r: r, isAktif: false, db: _dbService, user: _currentUserName, onRefresh: () => setState(() {}))),
                        ],
                      ]),
                    ),
                  );
  ...
  Widget _ziyaretciGorunumu(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.lock_person_outlined, size: 100, color: Colors.grey[300]),
          const SizedBox(height: 20),
          const Text("Giriş yaparak randevularınızı yönetin."),
          const SizedBox(height: 30),
          ElevatedButton(onPressed: () => Navigator.pushNamed(context, '/giris'), child: const Text("GİRİŞ YAP")),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Text(baslik, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, letterSpacing: -0.5)),
      ],
    );
  }
}
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          if (isAktif) 
                            const Icon(Icons.notifications_active_outlined, color: Colors.green, size: 18),
                        ],
                      ),
              ...
              Container(
                ...
                child: Center(
                  ...
                  ? IconButton(
                      ...
                      onPressed: () => _trendyolOylamaPopup(context),
                    )
                  : IconButton(
                      icon: Icon(oylandi ? Icons.star : Icons.star_border, 
                        color: oylandi ? Colors.amber : Colors.grey, size: 30),
                      onPressed: oylandi ? null : () => _trendyolOylamaPopup(context),
                    ),
                ),
              ),
            ],
              ...
              children: [
                Row(
                  ...
                  children: List.generate(5, (i) => IconButton(
                    icon: Icon(i < (step == 1 ? sP : uP) ? Icons.star_rounded : Icons.star_outline_rounded, 
              ...
              ElevatedButton(
                ...
                onPressed: (step == 1 ? sP : uP) == 0 ? null : () async {
                  if (step == 1) { setS(() => step = 2); }
                    ...
                    await db.yorumKaydet(
                      randevuId: r['id'] ?? "",
                      ustaIsmi: r['ustaIsmi'] ?? "",
                      salonIsmi: r['berberIsmi'] ?? "",
                      musteriAd: user ?? "Müşteri",
                      salonPuan: sP, salonYorum: sC.text,
                      ustaPuan: uP, ustaYorum: uC.text,
                    );
